...
//				fgets(str,4096,fp);
	...
	if (cmbPostEQ2->Items->Count > 0) {
		chkPostEQ2->Enabled = true;
		cmbPostEQ2->Enabled = true;
	}
...
//						}
	...
	cmbHfaOverrap->ItemIndex = 0;

	chkEnableCenterHFC->Checked = false;
	edtCenterHFC->Text = "";

	abe_10x_random_noise->Checked = false;

//	UpD_WltCutLevel->Position = 2;
	UpD_CutLevel2->Position = 0;
	...
	for (;p1 != NULL;) {
		...
		if (sscanf(p1,"-post_eq2:%d",&d1) == 1 && (d1 == -1 || (d1 >= 0 && d1 < 1000))) {
			if (d1 >= 0) {
				int n;
				for (i = 0;i < memo_eq_title->Lines->Count;i++) {
					sStr = memo_eq_title->Lines->Strings[i];
					if (sscanf(sStr.c_str(),"%d.%[^\n]",&n,title) == 2) {
						if (d1 == n) {
							cmbPostEQ2->ItemIndex = i;
							edt_PostEQ2_Title->Text = (UnicodeString)title;
							break;
						}
					}
				}
		...
		if (sscanf(p1,"-enable_post_eq2:%d",&d1) == 1 && d1 == 1) chkPostEQ2->Checked = true;
...
//		}
		if (sscanf(p1,"-abe_10x_cut_level_db:%d",&d1) == 1 && (d1 >= -20 && d1 <= 20)) {
			UpD_CutLevel2->Position = d1;
		}
//		if (sscanf(p1,"-abe_10x_cut_mode:%d",&d1) == 1 && (d1 >= 0 && d1 <= 2)) {
//  			cmbCutMode->ItemIndex = d1;
//		}
		if (sscanf(p1,"-abe_10x_random_noise:%d",&d1) == 1 && d1 == 1) {
			abe_10x_random_noise->Checked = true;
		}
		...
		if (sscanf(p1,"-hfa3_fft_window_size:%d",&d1) == 1 && (d1 == 0 || d1 >= 10 && d1 <= 60)) {
			if (d1 == 0) {
				cmbHFA3_FFT_Window_Size->ItemIndex = 0;
			} else {
				int ii;
				for (ii = 1;ii < cmbHFA3_FFT_Window_Size->Items->Count;ii++) {
					if (cmbHFA3_FFT_Window_Size->Items->Strings[ii].ToInt() == d1) {
						break;
					}
				}
				if (ii < cmbHFA3_FFT_Window_Size->Items->Count) {
					cmbHFA3_FFT_Window_Size->ItemIndex = ii;
				}
			}
		}
		...
		if (sscanf(p1,"-hfa3_fft_window_per_1s:%d",&d1) == 1 && (d1 >= 0 && d1 <= 2)) {
			cmbHfaOverrap->ItemIndex = d1;
		}
		...
		if (sscanf(p1,"-ovwrite_hfa:%d",&d1) == 1 && d1 >= 0 && d1 <= 4) {
			cmbHFA->ItemIndex = d1;
		}
...
void __fastcall TForm1::btnSaveClick(TObject *Sender)
{
	...
	if (chkPostEQ2->Checked == true) {
		uStr += " -enable_post_eq2:1";
	}
	...
	if (cmbPostEQ->ItemIndex >= 0) {
		uTemp.sprintf(L" -post_eq2:%s",cmbPostEQ2->Items->Strings[cmbPostEQ2->ItemIndex]);
		uStr += uTemp;
	}
...
//	uTemp.sprintf(L" -abe_10x_cut_level:%d",UpD_CutLevel->Position);uStr += uTemp;
	uTemp.sprintf(L" -abe_10x_cut_level_db:%d",UpD_CutLevel2->Position);uStr += uTemp;
//	uTemp.sprintf(L" -abe_10x_cut_mode:%d",cmbCutMode->ItemIndex);uStr += uTemp;
	uTemp.sprintf(L" -abe_10x_random_noise:%d",abe_10x_random_noise->Checked == true ? 1 : 0);uStr += uTemp;
...
//	uTemp.sprintf(L" -abe_wlt_level:%d",UpD_WltCutLevel->Position);uStr += uTemp;
	...
	uTemp.sprintf(L" -hfa3_fft_window_per_1s:%d",cmbHfaOverrap->ItemIndex);uStr += uTemp;
...
void __fastcall TForm1::cmbPostEQ2Change(TObject *Sender)
{
	FILE *fp;
	UnicodeString uFileName;
	UnicodeString uStr;
				...
				if (fgets(str,4096,fp) != NULL) {   			/* EQ(Type1) */
					if (fgets(str,4096,fp) != NULL) {			/* EQ(Type2) */
						if (fgets(str,4096,fp) != NULL) {		/* EQ(Type3) */
							if (fgets(str,4096,fp) != NULL) {	/* Title */
								p = strrchr(str,'\n');if (p) *p = '\0';
								edt_PostEQ2_Title->Text = (UnicodeString)str;
							}
						}
					}
				}
